...
"""
    RISCHIO: UNA DELLE SPECIE DELLA REGOLA CHE STO INCREMENTANDO INCREMENTA ANCHE 
    UN'ALTRA SPECIE CHE è ANCH'ESSA PRESENTE NELLA LEGGE CINETICA: ANALIZZARE!! 
    CASO MAI NON USARE QUELLA SPECIE PER L'ANALISI ??? MA SE SONO TUTTE COSì??
"""
...
"""
def withBoundary(rr, model, specie, reaction, nTest, valueIncrease):
    #original quantities of species
    originalQuantities = rr.model.getBoundarySpeciesConcentrations()
    ...
    role=analysisBoundary(rr, model, reaction, nTest, originalQuantities, valueIncrease, model.getSpecies(model.getReaction(reaction).getModifier(specie).getSpecies()))


    #reset the original quantities of species
    for (i,specieId) in enumerate(rr.model.getBoundarySpeciesIds()):
        if model.getAssignmentRule(specieId) is None:   
            rr.model.setBoundarySpeciesConcentrations([i], [originalQuantities[i]])
    ...
    for j in range(nTest):
        ...
        for i in range(0,len(quantities)):
            quantities[i]=quantities[i]+0.1+j
            rr.model.setBoundarySpeciesConcentrations([i],[quantities[i]])
        ...
        #analyze the role of modifier and reset the original quantities of species
        role=analysisBoundary(rr, model, reaction, nTest, quantities, valueIncrease, model.getSpecies(model.getReaction(reaction).getModifier(specie).getSpecies()))
        for (i,specieId) in enumerate(rr.model.getBoundarySpeciesIds()):
            if model.getAssignmentRule(specieId) is None:   
                rr.model.setBoundarySpeciesConcentrations([i], [originalQuantities[i]])
...
def withFloating(rr, model, specie, reaction, nTest, valueIncrease):
    #original quantities of species
    originalQuantities = rr.model.getFloatingSpeciesAmounts()
    ...
    role=analysisFloating(rr, model, reaction, nTest, valueIncrease, model.getSpecies(model.getReaction(reaction).getModifier(specie).getSpecies()))
