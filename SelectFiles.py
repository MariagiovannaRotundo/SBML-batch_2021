...
def selectWrong(rr, dirToExamine, dirNotAnalyze, others):
    ...
    for file in files:
        if file.endswith('.xml'):
            ...
            #we can't analyze in a correct way models which are bad read from libroadrunner.
            if  model.getNumSpecies()!=(rr.model.getNumBoundarySpecies() + rr.model.getNumFloatingSpecies()) or \
...
def selectQualitative(rr, dirToExamine, dirNotAnalyze, others):
...
def selectRules(rr, dirToExamine, dirNotAnalyze, others):
...
def selectEvents(rr, dirToExamine, dirNotAnalyze, others):
    ...
    for file in files:
        if file.endswith('.xml'):
            ...
            #we don't want to analyze models with rules or events or qualitative models
            #we can't analyze in a correct way models which are bad read from libroadrunner.
            if  model.getNumRules()>0 and model.getNumEvents()>0:
                shutil.move(dirToExamine+'\\'+file, dirNotAnalyze)
                print('To not analyze')            
        else:
            #we don't care other files
